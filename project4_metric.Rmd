...
```{r}
ratings <- read.csv('/home/alainkuiete/Documents/DATA612/movie20m/ml-20m/ratings.csv')
```
...
```{r}
(ratingShort <- mratings[rowCounts(mratings) > 900, colCounts(mratings) > 100])
```
...
```{r}
ratingShort <- mratings[, colCounts(mratings) > 100]
ratingShort <- ratingShort[rowCounts(ratingShort) > 900, ]
ratingShort
```
...
```{r}
# df <- as.data.frame(as.matrix(ratingShort@data))
# df$userId <- rownames(df)
# df <- df %>% gather(key = movieId, value = rating, -userId) %>% filter(rating != 0)
# write.csv(df, '/home/alainkuiete/Documents/DATA612/movie20m/ml-20m/ratings_final.csv', row.names = FALSE)
```
